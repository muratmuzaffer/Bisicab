...
pdf(fs.readFileSync(pdfPath)).then(({ text }) => {
  ...
  for (const { body } of blocks) {
    ...
    if (name.includes('Alper ARICA')) console.log('Alper days', days.slice(0, 10).join(','));
  }

  for (let d = 0; d < 5; d++) {
    console.log(`Day ${d + 1}: B1 ${b1[d]}/${summary.b1[d]} F1 ${f1[d]}/${summary.f1[d]}`);
  }
  console.log('Day1 total', b1[0] + f1[0], 'exp', summary.b1[0] + summary.f1[0]);
});
